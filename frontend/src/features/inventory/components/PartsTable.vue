...
// all pages — selection itself is still tracked globally (a plain array
// of SKU ids), so checking boxes on page 1, then page 2, accumulates
// correctly.
const allOnPageSelected = computed(() =>
  store.skus.length > 0 &&
  store.skus.every((row) => store.isSelected(row.sku))
);

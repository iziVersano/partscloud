...
- **Frontend test layer** (`feat/frontend-test-layer`, PR #16) — adds Vitest + Vue Test Utils (none existed before). New `frontend-tests` compose service runs alongside the backend `tests` service. Covers `inventoryStore.js` (load, filter, sort, selection, act/bulkAct including failure paths) and two component interactions (FilterBar click, PartsTable sort/Accept). Run locally with `npm test` from `frontend/`.

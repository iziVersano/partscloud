...
NOTE: risk logic and row validation are intentionally inlined here rather
than imported from services/risk.py and services/importer.py. Django's
migration framework executes migrations against historical model states,
and importing live app code in a migration creates a fragile dependency —
if a service signature ever changes, this historical migration would
break. Both are short enough that inlining is the correct pattern.

Row validation is deliberately lenient compared to services/importer.py
(used by the `import_skus` management command for ad hoc re-imports):
a row with a blank or non-numeric required field, or a nonsensical value
(negative stock, negative lead time, etc.), is skipped rather than
crashing the whole seed. Skipped rows are printed so a bad seed file is
noticed, not silently incomplete.
"""
import csv
...
def _parse_row(row):
    """Validate and parse one CSV row. Raises ValueError on bad data."""
